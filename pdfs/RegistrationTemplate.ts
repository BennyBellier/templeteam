...
export async function generateRegistrationPDF(data: RegistrationProps): Promise<Buffer> {
  try {
  const uint8Array = fs.readFileSync(path.join(process.cwd(), "public", "static", "Template_dossier_inscription_2024-2025.pdf"));
  ...
  const centuryGothicPath = path.join(process.cwd(), "fonts", "centurygothic.ttf");
  const centuryGothicBoldPath = path.join(process.cwd(), "fonts", "centurygothic_bold.ttf");
   ...
   */
    ...
    const signatureBytes = Uint8Array.from(atob(base64Data), char => char.charCodeAt(0));
    ...
    */
    ...
    page?.drawText(genDate.toLocaleDateString('fr-FR', {
      day: "numeric",
      month: "long",
      year: "numeric",
    }), {
  ...
  try {
    fs.writeFileSync(path.join(process.cwd(), "public", "static", 'output.pdf'), pdfBytes);
  } catch (err) {
