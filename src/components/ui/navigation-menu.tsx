...
>(({ className, children, ...props }, ref) => (
  <NavigationMenuPrimitive.Root
    ref={ref}
    className={cn(
      "relative z-10 flex h-fit items-center lg:justify-center",
      className,
    )}
    {...props}
  >
...
>(({ className, ...props }, ref) => {
  return (
    <NavigationMenuPrimitive.List
      ref={ref}
      className={cn(
        "group flex flex-1 list-none justify-center lg:items-center lg:space-x-1",
        className,
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div className={cn("absolute top-full flex justify-center")} style={style}>
      <NavigationMenuPrimitive.Viewport
        className={cn(
          "origin-top-center relative mt-1 h-[var(--radix-navigation-menu-viewport-height)] w-[var(--radix-navigation-menu-viewport-width)] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 ",
          className,
        )}
        ref={ref}
        {...rest}
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Indicator
    ref={ref}
    className={cn(
      "top-full z-[1] flex h-[0.350rem] items-end justify-center overflow-hidden duration-500 data-[state=visible]:animate-in data-[state=hidden]:animate-out data-[state=hidden]:fade-out data-[state=visible]:fade-in",
      className,
    )}
    {...props}
  >
